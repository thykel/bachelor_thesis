...
\chapter{Problems of The Current Internet}\label{problems}
    ...
    \section{Incomplete naming scheme}\label{problems:naming}
        ...
        In 1982, Jerome Saltzer in his work ``On the Naming and Binding of Network Destinations'' \cite{rfc1498} described the entities and the relationships that make a complete naming and addressing schema in networks. According to Saltzer, there are four elements that need to be identified: \texttt{applications}, \texttt{nodes}, \texttt{points of attachment} to the network (PoAs) and \texttt{paths}. The relationships between them are illustrated in Figure \ref{fig:saltzer}. At the time, network architectures such as CYCLADES \cite{Cyclades}, XNS, DECNET and OSI conformed to this scheme \cite{internet_demo}.
    ...
    \section{Lack of Mobility}\label{problems:mobility}
        ...
        \begin{itemize}
            ...
            This approach is used by technologies such as Named Data Networking (NDN) \cite{ndn} (described in section \ref{archs:ndn}).
        \end{itemize}
...
\chapter{Alternative Network Architectures}\label{archs}
        ...
        \subsection{Concepts}
            ...
            \subsubsection{The Building Blocks}
                ...
                The NDN architecture specifies:
                    \begin{itemize}
                        \item two types of packets: an \texttt{interest packet} (containing the name of desired data) and a \texttt{data packet} (containing the requested data),
                        \item two types of hosts: a \texttt{consumer} (data requester) and a \texttt{producer} (data provider), and
                        \item a \texttt{router} maintaining three fundamental data structures:
                        \begin{itemize}
                            \item \texttt{Forwarding Information Base} (FIB) (forwarding table)
                            \item \texttt{Pending Interest Table} (PIT) (maintaining currently active requests)
                            \item \texttt{Content Store} (data cache)
                        \end{itemize}
                        \begin{figure}[H]
                            \begin{center}
    ...
    \section{MobilityFirst}\label{archs:mf}
        ...
        \subsection{Concepts}
            ...
            \subsubsection{Name resolution}
                ...
                The system encompassing both the name resolution service and the name certification service is called the \texttt{Global Name System} (GNS).
    ...
    \section{eXpressive Internet Architecture}\label{archs:xia}
        ...
        \subsection{Concepts}
            ...
            \subsubsection{Principal Types}
                ...
                An \texttt{XIA principal} is specified by the semantics of communicaton between principals, the processing that is required to forward traffic with addresses of its type and the intristic security properties. The initial XIA architecture defines four basic XIA principal types:
        ...
        \subsection{Current State of Implementation}
            ...
            \begin{itemize}
                \item A Click-based prototype, including the XIA protocol stack, a routing daemon, a nameserver and an API.
    ...
    \section{Recursive InterNetwork Architecture}\label{archs:rina}
            ...
            \subsubsection{IPC Process}
                ...
                \begin{itemize}
                    ...
                    \begin{itemize}
                        ...
                        \item \textbf{\emph{Relaying and Multiplexing Task} (RMT).} A stateless function that 1) retrieves PDUs from EFCP instances and management tasks and multiplexes then onto a common (N-1)-flow, and 2) takes incoming PDUs and relays them within current IPC or passes them to outgoing port(s).
                    \end{itemize}
        ...
        \subsection{Current State of Implementation}
            \begin{itemize}
            \item \texttt{ProtoRINA}, a Java SDK used as the reference implementation.
            \item \texttt{IRATI Stack}, a native GNU/Linux implementation of RINA.
            \item \texttt{RINASim}, an open-source OMNeT++ implementation of RINA, developed by FIT BUT.
            \end{itemize}
...
\chapter{Testing and Evaluation}\label{testing}
    ...
    \section{Basic Relaying}
        ...
        \subsection{Simulation}
            ...
            \item \textbf{t=14}: The \texttt{IPC1}'s RIB daemon receives the \texttt{M\_CREATE\_RESPONSE} request and the \emph{Allocate(\texttt{IPC33})} procedure is sucesfully completed. The new connection is mapped to a newly instantiated RMTPort called \texttt{p0} and the previously suspended \emph{Allocate(\texttt{App2})} procedure is triggered to continue and carry out the same series of events, only one level higher and using \texttt{IPC1}1's port \texttt{p0} and its data queues. The \texttt{M\_CREATE} sent by\texttt{IPC11}'s RIB daemon is directed to \texttt{IPC33}'s RIB daemon.

            \item \textbf{t=16}: The RIB daemon sees that the \texttt{M\_CREATE} requires acces to \texttt{App2}, which isn't a local aplication, so the request has to be forwarded to \texttt{IPC22}. It holds the request and waits for Resource Allocator to process \emph{Allocate(\texttt{IPC22})}.
            ...
            The Forwarding policy returns a single RMTPort, \texttt{p1}.

            The dispatcher sends out the PDU through \texttt{p1}.
...
\chapter{Conclusion}\label{conclusion}
    ...
    The implementation goal of this thesis was to contribute to prototyping attempts of one of the presented network architectures, RINA. This task has been sucessfully completed by implementing RINA's Relaying and Multiplexing Task into an existing library for simulation tool OMNeT++.

    To be able to describe the principles of new network architectures and analyze their contributions, I needed to gain understanding of the driving factors behind their inception. This has been achieved by studying the Internet and its history to learn about choices that led to its current state, including its present problems. I have noticed that some of the drawbacks of its architectural design have been recognized right at the beginning of its deployment.
